...
public class SlimeyEffect extends MobEffect {
    ...
    public boolean applyEffectTick(@NotNull ServerLevel serverLevel, @NotNull LivingEntity livingEntity, int amplifier) {
        if (livingEntity.horizontalCollision) {
            Vec3 initialVec = livingEntity.getDeltaMovement(), climbVec = new Vec3(initialVec.x, 0.2D, initialVec.z);
            livingEntity.setDeltaMovement(climbVec.x * 0.91D, climbVec.y * 0.98D, climbVec.z * 0.91D);
            return true;
        }
